...
const { buildWelcomePanel } = require('./welcomeHandler');
const { handleMissionCommand } = require('./missionHandler');
const { handleHelpCommand } = require('./helpHandler');

function hasAdmin(msg) { return !!msg.member?.permissions.has("Administrator"); }

async function handleGangMessage(msg, command, args) {
  try {
    ...
    if (command === "gang") {
      ...
      if (!sub) return msg.reply("Usage: `?gang <create|join|leave|disband|info|members|bank|upgrade|promote|demote|mission>`");
      return handleGang(msg, sub, args.slice(1));
    }
  } catch (err) {
    console.error("Gang/Welcome command error:", err);
...
async function handleGang(msg, sub, args) {
  ...
  return msg.reply("Unknown subcommand. Use: `?gang <create|join|leave|disband|info|members|bank|upgrade|promote|demote|mission>`");
}
...
module.exports = { handleGangMessage };
