...
const { getWelcomeConfig, updateWelcomeConfig, resetWelcomeConfig } = require('../data/store');
...
async function handleWelcomeButton(interaction) {
  ...
  if (id === "welcome_test") {
    ...
    if (!cfg.channelId) return interaction.reply({ content: "❌ Set a welcome channel first.", ephemeral: true });
    const channel = await interaction.guild.channels.fetch(cfg.channelId).catch(() => null);
    if (!channel?.isTextBased()) return interaction.reply({ content: "❌ Welcome channel not found.", ephemeral: true });
...
async function handleWelcomeSelectMenu(interaction) {
  const guildId = interaction.guildId;
  const config  = getWelcomeConfig(guildId);
  const id      = interaction.customId;
  ...
  if (id === "welcome_select_color") {
    ...
    return interaction.update({ content: `🎨 Color set to **${config.embedColor}**!`, components: [] });
  }
  if (id === "welcome_select_channel") {
    config.channelId = interaction.values[0];
    updateWelcomeConfig(config);
    return interaction.update({ content: `📢 Welcome channel set to <#${config.channelId}>!`, components: [] });
  }
  if (id === "welcome_select_autorole") {
    config.autoroleId = interaction.values[0];
    updateWelcomeConfig(config);
    return interaction.update({ content: `👤 Auto-role set to <@&${config.autoroleId}>!`, components: [] });
  }
  if (id === "welcome_select_language") {
    config.language = interaction.values[0];
    updateWelcomeConfig(config);
    return interaction.update({ content: `🌐 Language set to **${config.language}**!`, components: [] });
  }
}
...
async function handleWelcomeModal(interaction) {
  ...
  if (id === "welcome_modal_image") {
    ...
    if (url && !isValidUrl(url)) return interaction.reply({ content: "❌ URL-ka waa inuu ku bilaabmaa `https://`", ephemeral: true });
  ...
  if (id === "welcome_modal_advanced") {
    ...
    updateWelcomeConfig(config);
    return interaction.reply({ content: `⚙️ Advanced updated! DM Welcome: **${config.dmWelcome}** | Anti-Bot: **${config.antiBot}**`, ephemeral: true });
  }
}

module.exports = { buildWelcomePanel, buildWelcomeEmbed, handleWelcomeButton, handleWelcomeSelectMenu, handleWelcomeModal };
