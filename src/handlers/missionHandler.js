...
const { getUserGang, updateGang } = require('../data/store');
...
async function handleMissionCommand(msg, args) {
...
async function startMission(msg, args) {
  ...
  if (activeMissions.has(gang.id))
    return msg.reply("❌ Your gang already has an active mission. Use `?gang mission status`.");
...
module.exports = { handleMissionCommand, MISSIONS };
